...
import type { DeclineTimeOffParams, TimeOffActionResponse } from '../client/timeoff.types';
...
export const declineTimeOffParameters = (_context?: Context) =>
  z.object({
    ...
    decline_reason: z.string().describe('The reason for declining the time off.'),
  });
...
export const declineTimeOff = async (
  ...
  try {
    const result = await apiClient.declineTimeOff(params as DeclineTimeOffParams);
    return result;
  } catch (error) {
