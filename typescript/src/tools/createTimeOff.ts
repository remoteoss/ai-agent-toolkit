...
import { TimeOffStatus, TimeOffType, TimeOffParams } from '../client/timeoff.types';
...
export const createTimeOffParameters = (_context?: Context) =>
  z.object({
    ...
    start_date: z.string().describe('The start date of the time off (YYYY-MM-DD).'),
    ...
    notes: z.string().optional().describe('Optional notes about the time off request.'),
    timeoff_days: z
      .array(
        z.object({
          day: z.string().describe('The specific day for the time off entry (YYYY-MM-DD).'),
          hours: z.number().describe('The number of hours for this specific day.'),
        }),
      )
...
export const createTimeOff = async (
  ...
  try {
    ...
    const approver =
      ...
      (await apiClient.listCompanyManagers({})).data.company_managers.at(0)?.userId;
