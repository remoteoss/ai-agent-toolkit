...
export const listTimeOffParameters = (
  ...
  z.object({
    employment_id: z.string().optional().describe('Filter time off for a specific employment ID.'),
    timeoff_type: z.string().optional().describe('Filter time off by its type (e.g., sick_leave, paid_time_off).'),
    status: z.string().optional().describe('Filter time off by its status (e.g., approved, requested).'),
