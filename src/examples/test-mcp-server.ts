...
async function main() {
  ...
  const serverArgs = ['src/mcp/cli.ts', `--api-key=${remoteApiKey}`, `--api-base-url=${remoteBaseUrl}`];

  console.log(`Starting MCP server with command: ${serverCommand} ${serverArgs.join(' ')}`);
  ...
  try {
    ...
    if (!availableTools.includes(listTimeOffToolName)) {
      console.error(`Error: Tool '${listTimeOffToolName}' not found in MCP server.`);
      return;
    }
    ...
    if (toolResult.content && Array.isArray(toolResult.content) && toolResult.content.length > 0) {
      const firstContent = toolResult.content[0] as { type?: string; text?: string }; // Type assertion
